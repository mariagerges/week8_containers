...
class Heap(BinaryTree):
    ...
    def __repr__(self):
        ...
        Recall that the __repr__ function
        should return a string that can be used to
    ...
    def is_heap_satisfied(self):
        ...
        This makes it possible to automatically test whether insert/
        delete functions
    ...
    def _is_heap_satisfied(node):
        ...
        left_sat = Heap._is_heap_satisfied(node.left)
        right_sat = Heap._is_heap_satisfied(node.right)

        if node.left:
            left = node.value <= node.left.value and left_sat
        if node.right:
            right = node.value <= node.right.value and right_sat
    ...
    def insert(self, value):
        ...
        representation of the total number of nodes
            ...
            heap in a variable (rather than compute it)
        ...
        '''
        if self.root:
            nodes_counter = self.__len__()
            insert_line = "{0:b}".format(nodes_counter + 1)[1:]
            self.root = Heap._insert(self.root, value, insert_line)
        else:
            self.root = Node(value)

    @staticmethod
    def _insert(node, value, insert_line):
        '''
        ...
        if insert_line[0] == '0':
            if not node.left:
                ...
                node.left = Heap._insert(node.left, value, insert_line[1:])
        if insert_line[0] == '1':
            if not node.right:
                node.right = Node(value)
            else:
                node.right = Heap._insert(node.right, value, insert_line[1:])
        if insert_line[0] == '0':
    ...
    def remove_min(self):
        ...
        if not self.root:
            ...
            nodes_counter = self.__len__()
            remove_line = "{0:b}".format(nodes_counter)[1:]
            last, self.root = Heap._remove_bottom_right(
                self.root, remove_line)
            if self.root:
    ...
    @staticmethod
    def _remove_bottom_right(node, remove_line):
        '''
        ...
        if len(remove_line) == 0:
            return None, None
        if remove_line[0] == '0':
            if len(remove_line) == 1:
                deleted_val = node.left.value
                node.left = None
            else:
                deleted_val, node.left = Heap._remove_bottom_right(
                    node.left, remove_line[1:])
        if remove_line[0] == '1':
            if len(remove_line) == 1:
                deleted_val = node.right.value
                node.right = None
            else:
                deleted_val, node.right = Heap._remove_bottom_right(
                    node.right, remove_line[1:])
        print(deleted_val, str(node))
        return deleted_val, node
